...
@router.get("/{user_id}/loans", summary="Consultar préstamos de un usuario")
def prestamos_usuario(user_id: int, db: Session = Depends(get_db)):
    from app.services.loan_service import listar_loans
    if user_service.buscar_usuario_por_id(db, user_id) is None:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")
    return [{"loan_id": x.id, "status": x.status, "loan_date": x.loan_date, "return_date": x.return_date,
             "device": {"id": x.device.id, "name": x.device.name, "serial_number": x.device.serial_number,
                        "device_type": x.device.device_type}} for x in listar_loans(db, user_id=user_id)]
